...
head(evals)
```
*Each row represents an entry by a specific student for a question on the evaluation survey for a specific course and teacher. So, there are multiple entries by the same students in the dataset, but each entry is a response to a different question* 
...
teacher_evals_clean |>
  ...
  group_by(teacher_id) |>
  mutate(avg_1_score = mean(SET_score_avg)) |> #New column with the average Q1 score for each teacher
  distinct(avg_1_score) |>
  arrange(desc(avg_1_score)) #Arrange by top average scores
...
teacher_evals_clean |>
  ...
  mutate(avg_1_score = mean(SET_score_avg)) |> #New column with the average Q1 score for each teacher
  distinct(avg_1_score) |>
  arrange(avg_1_score) #Arrange by top average scores
